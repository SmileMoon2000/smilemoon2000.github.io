var ECharts=[],EchartOptionList=[],myChart;
...
function calculateWalking(){
    ...
    var echartoption = Echart_Option("echart_disp","管道土阻力","x(m)","土阻力(N/m)",series_ResistanceSoil_H.concat(series_ResistanceSoil_C));
    EchartOptionList['resistancesoil']=echartoption;
    var echartoption = Echart_Option("echart_disp","管道轴力","x(m)","轴力(kN)",series_N_H.concat(series_N_C));
    EchartOptionList['N']=echartoption;
    var echartoption = Echart_Option("echart_disp","管道应变","x(m)","应变",series_strain_H.concat(series_strain_C));
    EchartOptionList['strain']=echartoption;
    var echartoption = Echart_Option("echart_disp","管道位移","x(m)","位移(m)",series_disp_H.concat(series_disp_C));
    // myChart.setOption(echartoption);ECharts.push(myChart);
    EchartOptionList['disp']=echartoption;
    ...
    var n_disp_Left =  linspace(0,5,11).map((key,value)=>[key,disp_Left[value]]);
    var n_disp_Right =  linspace(0,5,11).map((key,value)=>[key,disp_Right[value]]);
    var n_disp_Mid =  linspace(0,5,11).map((key,value)=>[key,disp_Mid[value]]);
    var n_disp_H =  linspace(0,5,11).map((key,value)=>[key,disp_H[value]]);
    var n_disp_C =  linspace(0,5,11).map((key,value)=>[key,disp_C[value]]);
    var series_disp = [
